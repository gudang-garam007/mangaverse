...
export function DailyReward({ onBerriesChange }: DailyRewardProps) {
  ...
  const checkDailyReward = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/bet/daily-reward`, {
      ...
      <button
        ...
        className={`relative px-6 py-3 rounded-xl font-bold transition-all flex items-center gap-2 ${
          claimed
            ...
            : "bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 text-white shadow-lg shadow-orange-900/30 animate-pulse"
        }`}
      >
